...
public class TabPedidoController {
    ...
    @GetMapping("/pedidos/{numped}")
    public ResponseEntity<Optional<Pedido>> getPedido(@PathVariable BigDecimal numped) {
        Optional<Pedido> resultado = pedidoRepository.findById(numped);
        return ResponseEntity.ok().body(resultado);
    }
    ...
    public ResponseEntity<TabPedido> marcarChegada(@PathVariable(value = "numped") BigDecimal numped)
            throws ResourceNotFoundException {
    ...
    @PutMapping("/tabpedidos/separacao/{numped}")
    public ResponseEntity<TabPedido> updateTabPedidoSeparacao(@PathVariable(value = "numped") BigDecimal numped,
            @Valid @RequestBody TabPedido tabPedidoDetails) throws ResourceNotFoundException {
        TabPedido tabPedido = tabPedidoRepository.findById(numped)
                .orElseThrow(() -> new ResourceNotFoundException("Pedido não encontrado com esse Numped :: " + numped));
    ...
    @PutMapping("/tabpedidos/conferencia/{numped}")
    public ResponseEntity<TabPedido> updateTabPedidoConferencia(@PathVariable(value = "numped") BigDecimal numped,
            @Valid @RequestBody TabPedido tabPedidoDetails) throws ResourceNotFoundException {
        TabPedido tabPedido = tabPedidoRepository.findById(numped)
                .orElseThrow(() -> new ResourceNotFoundException("TabPedido não encontrado com esse Numped :: " + numped));
        Pedido pedido = pedidoRepository.findById(numped)
                .orElseThrow(() -> new ResourceNotFoundException("Pedido não encontrado com esse Numped :: " + numped));
        
        tabPedido.setCodfuncbalcao(tabPedidoDetails.getCodfuncbalcao());
        if(tabPedidoDetails.getDatainiciobalcao() !=  null) {
            tabPedido.setDatainiciobalcao(new Date());
        };
        
        if(tabPedidoDetails.getDatafimbalcao() !=  null) {
            tabPedido.setDatafimbalcao(new Date());
            pedido.setDtfinalcheckout(new Date());
            pedido.setCodfuncconf(tabPedidoDetails.getCodfuncbalcao()) ; 
        };
